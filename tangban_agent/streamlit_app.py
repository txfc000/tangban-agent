...
# 确保 user_data 有 chat_archives 字段
if "chat_archives" not in user_data:
    user_data["chat_archives"] = []
...
with tab1:
    ...
    # ===== 智能生成方式 =====
        ...
        if not is_finished:
            ...
            st.success("✅ 食谱已生成，您可以在上方查看。如需重新生成，请再次点击“智能生成”选项。")
            if st.button("🔄 重新开始智能对话"):
                st.session_state.agent_messages = []
                st.rerun()
...
with tab3:
    ...
    # 初始化当前对话消息（从 user_data 加载）
    if "current_chat" not in st.session_state:
        # 从 user_data 加载当前对话（如果没有则新建）
        st.session_state.current_chat = user_data.get("chat_history", [])
...
"""
        try:
            ...
            # 取最近10条
            for m in st.session_state.current_chat[-10:]:
                messages.append(m)
            ...
            # 自动保存到 user_data（但不存档）
            user_data["chat_history"] = st.session_state.current_chat
            db.save_user_data(username, user_data)
            st.rerun()
    ...
    with col1:
        if st.button("📝 开启新话题"):
            ...
            if st.session_state.current_chat:
                # 保存到 user_data["chat_archives"]
                archive = {
                    "time": time.strftime("%Y-%m-%d %H:%M"),
                    "messages": st.session_state.current_chat.copy()
                }
                user_data.setdefault("chat_archives", []).append(archive)
                # 清空当前对话
    ...
    with st.expander("📚 历史对话记录"):
        archives = user_data.get("chat_archives", [])
        if archives:
            for idx, arch in enumerate(reversed(archives)):
                st.markdown(f"**📅 {arch['time']}**")
                # 显示前几条预览（可选）
                for msg in arch['messages'][-3:]:  # 显示最后3条
                    role = "👤 用户" if msg["role"] == "user" else "🤖 助手"
                    st.write(f"{role}: {msg['content'][:50]}...")
                # 添加“查看全部”和“删除”功能
                col_a, col_b = st.columns([1, 1])
                with col_a:
                    if st.button(f"查看全部", key=f"view_{idx}"):
                        st.session_state.view_archive = idx  # 可以用session_state展示详情
                        # 由于展开框内不能动态弹出，我们简单展示全部消息
                        st.info("完整对话如下：")
                        for m in arch['messages']:
                            role = "用户" if m["role"] == "user" else "助手"
                            st.write(f"**{role}**: {m['content']}")
                with col_b:
                    if st.button(f"🗑️ 删除", key=f"del_arch_{idx}"):
                        # 从列表中移除
                        user_data["chat_archives"].pop(len(archives) - 1 - idx)
                        db.save_user_data(username, user_data)
                        st.rerun()
                st.divider()
        else:
